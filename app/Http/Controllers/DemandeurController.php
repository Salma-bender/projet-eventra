    ...
    public function AjoutContact(Request $request)
{
    $this->validate($request, [
        ...
        'numero.digits' => 'Le numéro doit contenir exactement 8 chiffres.',
        'numero.required' => 'Le numéro est obligatoire.',
        'numero.numeric' => 'Le numéro doit être composé uniquement de chiffres.',
    ...
    public function downloadInvoice(Reservation $reservation)
    {
        ...
        $qrData .= "Prix: {$reservation->event->Prix}  DH\n";
        ...
        //==1$qrCode = QrCode::format('svg')->size(200)->generate($qrData);
       
        $qrCode = QrCode::format('svg')->size(200)->generate($qrData);
$qrCodeBase64 = base64_encode($qrCode);
$pdf = PDF::loadView('Client.invoices.invoice-template', [
...
return $pdf->download('Facture-' . $reservation->id . '.pdf');
        ...
        // Generate the PDF using the invoice template view
         $pdf = PDF::loadView('Client.invoices.invoice-template', compact('reservation', 'qrCodeBase64'));
...
{
    // Valide et enregistre l'avis
    $request->validate([
        'avis' => 'required|string|max:1000',
    ]);

    // Exemple d'enregistrement (à adapter selon ta table)
    \App\Models\Feedback::create([
        'user_id' => auth()->id(),
        'message' => $request->avis,
